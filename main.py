...
# The Raw Check Endpoint: For load testing and internal engine verification (No DB required)
@app.post("/check-raw")
def check_raw(submissions: List[Submission]):
    # Convert Pydantic models back to dicts for the scorer
    raw_data = [s.model_dump() for s in submissions]
    results = run_plagiarism_check(all_submissions=raw_data)
    return {
...
def check_plagiarism(request: PlagiarismRequest):
    ...
    for qid, pairs in sorted(by_question.items()):
        # Derive summary counts from the assigned label.
        # This correctly reflects the hybrid output of BOTH the Token and AST engines.
        summaries.append(
            QuestionSummary(
                question_id=qid,
                total_pairs_checked=len(pairs),
                exact_match=sum(1 for p in pairs if p["label"] == "Exact match"),
                near_identical_text=sum(
                    1 for p in pairs if p["label"] == "Near-identical text"
                ),
                high_token_overlap=sum(
                    1 for p in pairs if p["label"] == "High token overlap"
                ),
                ...
                moderate_structural_textual=sum(
                    1
                    for p in pairs
                    if p["label"] == "Moderate similarity — structural and textual"
                ),
                moderate_text_similarity=sum(
                    1 for p in pairs if p["label"] == "Moderate text similarity"
                ),
                slight_text_similarity=sum(
                    1 for p in pairs if p["label"] == "Slight text similarity"
                ),
            )
        )
